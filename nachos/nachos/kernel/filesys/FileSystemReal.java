...
class FileSystemReal extends FileSystem {
  ...
  private static int NumDirEntries = 10;
 ...
 public static OpenFile childDirectoryFile;
  
  Map<String, OpenFile> childDirectoryMap = new HashMap<>();
  
  private OpenFile currentDirectoryFile;
  ...
  protected FileSystemReal(DiskDriver diskDriver, boolean format) { 
    ...
    if (format) {
      ...
      currentDirectoryFile = directoryFile; // Current directory is root directory
	...
	directory.print();
      ...
      freeMapFile = new OpenFileReal(FreeMapSector, this);
      directoryFile = new OpenFileReal(DirectorySector, this);
      currentDirectoryFile = new OpenFileReal(DirectorySector, this);
    }
  }
  ...
  void readSector(int sectorNumber, byte[] data, int index) {
      //Debug.println('+', "inside the real filsys readSector sectorNumber is" + sectorNumber);
...
public boolean create(String name, long initialSize) {
    ...
    directory = new Directory(getNumDirEntries(), this);
    //call fetch in order to initialize it from the disk
    ...
    if (directory.find(name) != -1)
      ...
      //initialize the contents of bitmap
	...
	success = false;	// no space in directory because we check the file before
      else {
	  //Debug.println('+',"begin to create fileheader");
	  //add a file header to the new created file
	hdr = new FileHeader(this);
	//allocate data blocks for the file
	if (!hdr.allocate(freeMap, (int)initialSize)){
	  success = false;	// no space on disk for data
	  //Debug.println('+', "fail to allocate space");
	}
	else {	
	  success = true;
	  //Debug.println('+', "file test create!!!");
	  // everthing worked, flush all changes back to disk
	  hdr.writeBack(sector); 	
	  Debug.println('+', "name of the file is " +name);
	  directory.writeBack(currentDirectoryFile);
  ...
  /*This method is for creating the file inside the  child directory
   * */
  public boolean createChild(String name, long initialSize, String directoryName) {
    return false;
    }
  /*this method change the current directory to the sub directory*/
  public boolean moveToTheChildDirectory(String name){
      //get current directory
      Directory directory = new Directory(getNumDirEntries(), this);
      //call fetch in order to initialize it from the disk
      directory.fetchFrom(currentDirectoryFile);
      //get the directory we want
      int sector = directory.findDirectory(name);
      if (sector == -1){
	  Debug.println('+', "move to " + name+" fail");
	  return false;
      }
      else{
	  OpenFile childDirectoryFile = new OpenFileReal(sector, this);
	  this.setCurrentDirectoryFile(childDirectoryFile);
	  Debug.println('+', "move to " + name+" success");
	  return true;
      }
  }
  public void moveToRoot(){
      this.setCurrentDirectoryFile(directoryFile);
  }
...
public boolean remove(String name) { 
  ...
  public boolean mkdir(String name) {
      Directory newDirectory = new Directory(NumDirEntries, this); // Temporary variable used in RAM
      
      Directory parentDirectory;
      BitMap freeMap;
      FileHeader newDirHdr;
      int newDirSector;
      boolean success;

      Debug.printf('f', "Creating new directory %s\n", name);

      parentDirectory = new Directory(NumDirEntries, this);
      parentDirectory.fetchFrom(getCurrentDirectoryFile());
      if(parentDirectory.findDirectory(name)!= -1)
	  success = false;
      else{
	  freeMap = new BitMap(numDiskSectors);
	  freeMap.fetchFrom(freeMapFile);
	  newDirSector = freeMap.find();	// find a sector to hold the file header of new directory
	  if (newDirSector == -1) 		
	      success = false;		// no free block for file header of new directory
	  else if (!parentDirectory.addDirectory(name, newDirSector))
	      success = false;	// no space in directory
	  else {
	      newDirHdr = new FileHeader(this);
	      if (!newDirHdr.allocate(freeMap, DirectoryFileSize))
		  success = false;	// no space on disk for data
	      else {	
              success = true;
              // everthing worked, flush all changes back to disk
              newDirHdr.writeBack(newDirSector); 		
              parentDirectory.writeBack(getCurrentDirectoryFile());
              freeMap.writeBack(freeMapFile);
              
              //childDirectoryFile = new OpenFileReal(newDirSector, this);
              
              OpenFile newDirectoryFile = new OpenFileReal(newDirSector, this);
              
              //childDirectoryMap.put(name, newDirectoryFile); // add to map
  ...
  public boolean rmdir(String name) {
      ...
      directory = new Directory(NumDirEntries, this);
      directory.fetchFrom(getCurrentDirectoryFile());
      //find the sector for the directory
      sector = directory.findDirectory(name);
      if (sector == -1) {
         return false;			 // file not found 
      }
      
      Directory foundDirectory = new Directory(NumDirEntries, this);
      OpenFileReal directoryFile = new OpenFileReal(sector,this);
      foundDirectory.fetchFrom(directoryFile); // Open this directory

      
      if(!foundDirectory.isEmpty()){ // If this directory is not empty, we cannot remove this directory
	  //Debug.println('+', "lala "+foundDirectory.isEmpty());
	  //remove every thing in the directory is required
	  return false;
      }
      //after clearing everything inisde the directory, clear the directory itself
      ...
      //childDirectoryMap.remove(name); // Remove this directory from the map
      
      freeMap.writeBack(freeMapFile);		// flush to disk
      directory.writeBack(getCurrentDirectoryFile());        // flush to disk
      return true;
  }
  /**
...
public void list() {
  ...
  @Override
public void print() {
      Debug.println('+', "Start printing everything");
  ...
  public OpenFile getFreeMapFile(){
      return this.freeMapFile;
  }
  
  public static void setNumDirEntries(int num){
      NumDirEntries = num;
  }
  
  public static int getNumDirEntries(){
      return NumDirEntries;
  }
  
  public void setCurrentDirectoryFile(OpenFile openFile) {
      currentDirectoryFile = openFile;
      
  }
  
  public OpenFile getCurrentDirectoryFile() {
      return currentDirectoryFile;
  }
  
  public OpenFile getRootDirectoryFile() {
      return directoryFile;
  }
  
  public Map<String, OpenFile> getChildDirectoryMap() {
      return childDirectoryMap;
  }
}
