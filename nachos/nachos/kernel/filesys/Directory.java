...
class Directory {
    ...
    /** Table of pairs: directory name/directory file header location */
    private DirectoryEntry directTable[];
    
    private int directTableSize;
	...
	//Debug.println('+', "table size is " + tableSize);
	byte buffer[] = new byte[tableSize * DirectoryEntry.sizeOf()];
	int pos = 0;
	for (int i = 0; i < tableSize; i++) {
    ...
    //find the directory
    private int findIndexDirectory(String name){
	for(int i = 0; i< tableSize;i++){
	    if(table[i].inUse()&& name.equals(table[i].getName())&& table[i].getDirectoryFlag()==true){
		return i;
	    }
	}
	return -1;
	...
	if (findIndex(name) != -1)
	    return false;
	Debug.println('+', "current size is " + tableSize);
	for (int i = 0; i < tableSize; i++) {
	    if (!table[i].inUse()) {
	...
	Debug.println('+',"here over 10, so the make it more than 10");
	// Will not reach here, unless current table is all occupied and needs to double the size of array
	
	DirectoryEntry[] tem = new DirectoryEntry[tableSize+10]; // Double the array size
	this.tableSize = tableSize+10;
	FileSystemReal.setNumDirEntries(tableSize);
	for (int i = 0; i < tableSize; i++) { // Initialize the new array elements
	    tem[i] = new DirectoryEntry();
	}
	
	for(int i = 0; i < table.length; i++) { // Copy old array elements to new array
	    tem[i] = table[i];
	}
	
	table = tem; // Old array variable reference new array variable
	//Debug.println('+', "talbeSize is " + tableSize);
	for (int i = 0; i < tableSize; i++){
	    Debug.println('+', "lalala " + i);
	    if (!table[i].inUse()) {
		Debug.println('+', "hahaha ");
		if(!table[i].setUsed(name, newSector)){
		    return(false);
		}
		return(true);
    ...
    void list() {
  	for (int i = 0; i < tableSize; i++) {
  	    if (table[i].inUse() && table[i].getDirectoryFlag()==false){
  		System.out.println("/"+table[i].getName());
  	    }
  	    else if (table[i].inUse() && table[i].getDirectoryFlag()==true){
  		System.out.print("/"+table[i].getName());
  		OpenFile child = new OpenFileReal(table[i].getSector(),filesystem);
  		Directory childD = new Directory(filesystem.numDiskSectors,filesystem);
  		childD.fetchFrom(child);
  		if(childD.isEmpty()){
  		  System.out.println("/");
  		}
  		else{
    ...
    void listSub(String dirName) { // 2nd layer
  	if(isEmpty())
  	    System.out.println("/"+dirName+"/");
  	    
  	for(int i = 0; i < tableSize; i++) {
  	    if (table[i].inUse()) {
  		System.out.println("/"+dirName+"/"+table[i].getName());
  	    }
  	}
      }
