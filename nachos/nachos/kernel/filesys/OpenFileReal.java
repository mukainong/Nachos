 ...
 */
public class OpenFileReal implements OpenFile {
	...
	//store the current write position of the file
	seekPosition += result;
	return result;
    }
	...
	//Debug.println('+', "seekPosition is " + seekPosition + " inside the open file real");
	...
	//Debug.println('+', "inside openfilereal " + firstSector  +" " + lastSector );
	for (i = firstSector; i <= lastSector; i++){
	    //Debug.println('+', "open file real is" +hdr.byteToSector(i * diskSectorSize) +" i is " + i);
	...
	if ((position + numBytes) > fileLength){ 
	 ...
	 //numBytes = fileLength - (int)position; 
	  //allocate space for the file and increase the file length
	    BitMap freeMap;
	    freeMap = new BitMap(filesystem.numDiskSectors);
	    freeMap.fetchFrom(filesystem.getFreeMapFile());
	    Debug.println('+',"Add additional space");
	    //allocate data blocks for the file
	    boolean add = hdr.allocateAdditional(freeMap, (int)position + numBytes - fileLength);
	    if (add == false){
		Debug.println('+', "fail to add space");
		return 0;
	    }
	    //write sector
	    else 
		Debug.println('+', "add space successfully");
	    hdr.writeBack(this.sector);
