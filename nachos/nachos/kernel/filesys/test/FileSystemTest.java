...
//	}
    ...
    private static final int FileSize = ContentSize * 10000;
	...
	//create the file the contents do not pass to the file at present
	if (!Nachos.fileSystem.create(FileName, FileSize)) {
	    Debug.printf('+', "Perf test: can't create %s\n", FileName);
	    return;
	}
	String caozhi = "caozhi";
	
	for(int k = 0; k<15; k++){
	    if (!Nachos.fileSystem.create(caozhi + k, 200)) {
		    Debug.printf('+', "Perf test: can't create %s\n", FileName+k);
	    }
	}
	...
	//remove need to be implemented
	//Nachos.fileSystem.remove("caozhi1");
	//write into the file
	String ContentString1 = "123456789x";
	String ContentString2 = "123456789y";
	//String ContentString3 = "123456789z";
	byte Contents1[] = ContentString1.getBytes();
	byte Contents2[] = ContentString2.getBytes();
	//byte Contents3[] = ContentString3.getBytes();
	Debug.println('+', "start writing");
	for (i = 0; i < FileSize; i += ContentSize) {
	    
	    if((i/ContentSize) % 2 == 1){
		Debug.println('z', "write " + ContentString1 +" " + i);
	    numBytes = openFile.write(Contents1, 0, ContentSize);
	    }
	    else{
		Debug.println('z', "write " + ContentString2 +" " + i);
		numBytes = openFile.write(Contents2, 0, ContentSize);
	    }
	    //Debug.println('+', ""+i);
	    if (numBytes < 10) {
	...
	Debug.println('+', "write additional things");
	byte things[] = "abc".getBytes();
	numBytes = openFile.write(things, 0, 3);
	Debug.println('+', "delete the huge file");
	Nachos.fileSystem.remove(FileName);
	
	openFile = Nachos.fileSystem.open("caozhi0");
	
	for (i = 0; i < 5000; i += ContentSize) {
	...
	//test directory
	Debug.println('+', "start testing directory\n\n");
	
	if (!Nachos.fileSystem.mkdir("kun")) {
	    Debug.println('+', "cannot create kun");
	    return;
	}
	
	if (!Nachos.fileSystem.mkdir("kun1")) {
	    return;
	}
	
	if (!Nachos.fileSystem.rmdir("kun1")) {
	    
	}
	
	if (!Nachos.fileSystem.mkdir("kun2")) {
	    
	}
	
	if (!Nachos.fileSystem.mkdir("kun3")) {
	    
	}
	
	//Nachos.fileSystem.setCurrentDirectoryFile(Nachos.fileSystem.getChildDirectoryMap().get("kun3")); // set directory to kun 3
	
	Nachos.fileSystem.mkdir("kun4");
	Nachos.fileSystem.moveToTheChildDirectory("kun4");
	Nachos.fileSystem.mkdir("kun5");
	Nachos.fileSystem.moveToTheChildDirectory("kun5");
	Nachos.fileSystem.create("mukai", 20);
	Nachos.fileSystem.moveToRoot();    
	...
	if ((openFile = Nachos.fileSystem.open("caozhi0")) == null) {
	...
	for (i = 0; i < FileSize; i += ContentSize) {
	    ...
	    Debug.println('z', "read " + string +" "+ i +" " + numBytes);
	    if ((numBytes < 10)) {
	...
	numBytes = openFile.read(buffer2, 0, 3);
	String string = null;
	try {
		string = new String(buffer2, "UTF-8");
	    } catch (UnsupportedEncodingException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	    }
	Debug.println('+',"read " +string +" "+ numBytes);
    }
	...
	NachosThread thread = new SubNachosThread("Filesystem test", new FileSystemTest());
	Debug.println('+', "file test");
